...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  #pragma omp parallel shared(train_dataset, layer_mtdt_arr, layer_dims_arr, num_layers, learning_rate, output_layer_activations, xentropy_derivative, backprop_params_arr, streams, grad_descent_params_arr) default(none)
  {
