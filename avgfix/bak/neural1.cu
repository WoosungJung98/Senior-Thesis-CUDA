...
layer_metadata_t* init_layer_metadata_arr(uint32_t* layer_dims_arr, uint32_t num_layers, uint32_t batch_size) {
  ...
  for(i=1; i<num_layers; i++) {
    ...
    cudaMemcpy(layer_mtdt_arr[i].avg_actv_deriv, init_deltas, curr_dim * sizeof(FP), cudaMemcpyHostToDevice);
    free(init_deltas);
...
__global__ void update_deltas_gpu(FP* avg_derivative, FP* avg_activation, int batch_size, FP* deltas, int n, int m, FP learning_rate) {
  // position within submatrix
  int C_col = threadIdx.x;
  int C_row = threadIdx.y;
  // absolute position
  ...
  if(C_row_abs < n && C_col_abs < m) {
    ...
    deltas[C_row_abs * m + C_col_abs] = Cval;
  }
}

void update_deltas(FP* d_avg_derivative, FP* d_avg_activation, int batch_size, FP* d_deltas, int n, int m, FP learning_rate) {
  dim3 Block(BLOCK_SZ, BLOCK_SZ);
  dim3 Grid;
  Grid.x = (m + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (n + BLOCK_SZ - 1) / BLOCK_SZ;
  
  update_deltas_gpu<<<Grid,Block>>>(d_avg_derivative, d_avg_activation, batch_size, d_deltas, n, m, learning_rate);
}

__global__ void update_biasdeltas_gpu(FP* avg_derivative, int batch_size, FP* biasdeltas, int n, int m, FP learning_rate) {
  // absolute position
  int C_col_abs = threadIdx.x + BLOCK_SZ * blockIdx.x;
  int C_row_abs = threadIdx.y + BLOCK_SZ * blockIdx.y;
  FP Cval;
  ...
  if(C_row_abs < n && C_col_abs < m) {
    ...
    biasdeltas[C_row_abs] = Cval;
  }
}

void update_biasdeltas(FP* d_avg_derivative, int batch_size, FP* d_biasdeltas, int n, int m, FP learning_rate) {
  dim3 Block(BLOCK_SZ, BLOCK_SZ);
  dim3 Grid;
  Grid.x = (m + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (n + BLOCK_SZ - 1) / BLOCK_SZ;
  
  update_biasdeltas_gpu<<<Grid,Block>>>(d_avg_derivative, batch_size, d_biasdeltas, n, m, learning_rate);
}
...
void minibatch_backprop(FP* d_curr_avg_deriv, FP* d_next_avg_deriv, FP* d_xentropy_derivative, uint32_t batch_size, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    update_deltas(d_curr_avg_deriv, layer_mtdt_arr[i - 1].avg_activation, batch_size, layer_mtdt_arr[i].deltas, curr_dim, next_dim, learning_rate);
    update_biasdeltas(d_curr_avg_deriv, batch_size, layer_mtdt_arr[i].biasdeltas, curr_dim, 1, learning_rate);
  ...
  update_deltas(d_curr_avg_deriv, layer_mtdt_arr[0].avg_activation, batch_size, layer_mtdt_arr[1].deltas, layer_dims_arr[1], layer_dims_arr[0], learning_rate);
  update_biasdeltas(d_curr_avg_deriv, batch_size, layer_mtdt_arr[1].biasdeltas, layer_dims_arr[1], 1, learning_rate);
}
