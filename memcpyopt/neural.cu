...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  extract_single_example<<<Grid,Block>>>(d_layer_output_single, layer_dim, 1, d_layer_outputs, batch_size, idx_in_batch, d_max_val, d_sum);

  calc_single_output_max<<<Grid,Block>>>(d_layer_output_single, layer_dim, 1, d_max_val);

  calc_single_output_sum<<<Grid,Block>>>(d_layer_output_single, layer_dim, 1, d_max_val, d_sum);

  Grid.x = (layer_dim + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (layer_dim + BLOCK_SZ - 1) / BLOCK_SZ;
  calc_softmax_jacobian<<<Grid,Block>>>(d_softmax_jacobian, layer_dim, layer_dim, d_layer_output_single, d_max_val, d_sum);
...
FP* calc_relu_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  Grid.x = (1 + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (layer_dim + BLOCK_SZ - 1) / BLOCK_SZ;
  extract_single_example<<<Grid,Block>>>(d_layer_output_single, layer_dim, 1, d_layer_outputs, batch_size, idx_in_batch, NULL, NULL);

  calc_relu_derivative_gpu<<<Grid,Block>>>(d_relu_derivative, layer_dim, 1, d_layer_output_single);
...
void stochastic_backprop(void* backprop_params) {
  ...
  Grid.x = (1 + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (MNIST_LABELS + BLOCK_SZ - 1) / BLOCK_SZ;
  extract_single_example<<<Grid,Block>>>(d_curr_derivative, MNIST_LABELS, 1, d_xentropy_derivative, batch_size, idx_in_batch, NULL, NULL);
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    calc_hadamard_product<<<Grid,Block>>>(d_curr_derivative, 1, next_dim, d_next_derivative, d_activation_derivative);
  }
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  #pragma omp parallel shared(train_dataset, layer_mtdt_arr, layer_dims_arr, num_layers, learning_rate, d_labels, xentropy_args, output_layer_activations, d_xentropy_derivative, backprop_params_arr, streams, backprop_args_arr, actv_deriv_args_arr, grad_descent_params_arr) default(none)
  {
