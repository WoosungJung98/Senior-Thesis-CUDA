...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  extract_single_example<<<Grid,Block,0,*stream>>>(d_layer_output_single, layer_dim, 1, d_layer_outputs, batch_size, idx_in_batch, d_max_val, d_sum);
...
FP* calc_relu_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  Grid.x = (1 + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (layer_dim + BLOCK_SZ - 1) / BLOCK_SZ;
  extract_single_example<<<Grid,Block,0,*stream>>>(d_layer_output_single, layer_dim, 1, d_layer_outputs, batch_size, idx_in_batch, NULL, NULL);
...
void stochastic_backprop(void* backprop_params) {
  ...
  Grid.x = (1 + BLOCK_SZ - 1) / BLOCK_SZ;
  Grid.y = (MNIST_LABELS + BLOCK_SZ - 1) / BLOCK_SZ;
  extract_single_example<<<Grid,Block,0,*stream>>>(d_curr_derivative, MNIST_LABELS, 1, d_xentropy_derivative, batch_size, idx_in_batch, NULL, NULL);
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    Grid.y = (1 + BLOCK_SZ - 1) / BLOCK_SZ;
    blocked_gpu_matrixmult<<<Grid,Block,0,*stream>>>(d_curr_derivative, layer_mtdt_arr[i].weights, d_next_derivative, NULL, 1, next_dim + 1, curr_dim, false);
    ...
    calc_hadamard_product<<<Grid,Block,0,*stream>>>(d_curr_derivative, 1, next_dim, d_next_derivative, d_activation_derivative);
  }
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  for(k=0; k<NUM_STREAMS; k++) {
    ...
    grad_descent_params_arr[k].d_labels = d_labels;
    grad_descent_params_arr[k].xentropy_args = &xentropy_args;
    grad_descent_params_arr[k].output_layer_activations = output_layer_activations;
    grad_descent_params_arr[k].d_xentropy_derivative = d_xentropy_derivative;
    grad_descent_params_arr[k].backprop_params_arr = backprop_params_arr;
    grad_descent_params_arr[k].streams = streams;
    grad_descent_params_arr[k].backprop_args_arr = backprop_args_arr;
    grad_descent_params_arr[k].actv_deriv_args_arr = actv_deriv_args_arr;
