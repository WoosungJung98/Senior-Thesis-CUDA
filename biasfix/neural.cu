...
layer_metadata_t* init_layer_metadata_arr(uint32_t* layer_dims_arr, uint32_t num_layers, uint32_t batch_size) {
  ...
  layer_mtdt_arr[0].biasdeltas = NULL;
  ...
  for(i=1; i<num_layers; i++) {
    ...
    free(init_weights);
    cudaMalloc((void**)&layer_mtdt_arr[i].deltas, prev_dim * curr_dim * sizeof(FP));
    cudaMalloc((void**)&layer_mtdt_arr[i].biasdeltas, curr_dim * sizeof(FP));
    FP* init_deltas = (FP*)malloc(prev_dim * curr_dim * sizeof(FP));
    for(k=0; k<prev_dim * curr_dim; k++) {
      init_deltas[k] = 0;
    }
    cudaMemcpy(layer_mtdt_arr[i].deltas, init_deltas, prev_dim * curr_dim * sizeof(FP), cudaMemcpyHostToDevice);
    cudaMemcpy(layer_mtdt_arr[i].biasdeltas, init_deltas, curr_dim * sizeof(FP), cudaMemcpyHostToDevice);
    free(init_deltas);
...
void free_layer_metadata_arr(layer_metadata_t* layer_mtdt_arr, uint32_t num_layers) {
  ...
  for(i=1; i<num_layers; i++) {
    ...
    cudaFree(layer_mtdt_arr[i].biasdeltas);
...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* streams, activation_deriv_args_t* args_arr) {
  ...
  if(curr_thread == 0) {
    ...
    for(k=0; k<NUM_STREAMS; k++) {
      if(idx_in_batch + k >= batch_size) break;
      blocked_gpu_matrixmult<<<Grid,Block,0,streams[k]>>>(args_arr[k].d_layer_output_single, args_arr[k].d_softmax_jacobian, args_arr[k].d_softmax_derivative, NULL, 1, layer_dim, layer_dim, false, false);
    }
  }
  #pragma omp barrier
...
__global__ void update_deltas_gpu(FP* curr_derivative, FP* activations, int idx_in_batch, int batch_size, FP* deltas, int n, int m, FP learning_rate) {
  ...
  if(C_row_abs < n && C_col_abs < m) {
    ...
    if(C_row == 0) {
      actv_shr[C_col] = activations[C_col_abs * batch_size + idx_in_batch];
    }
    if(C_col == 0)
...
__global__ void update_biasdeltas_gpu(FP* curr_derivative, int batch_size, FP* biasdeltas, int n, int m, FP learning_rate) {
  // absolute position
  int C_col_abs = threadIdx.x + BLOCK_SZ * blockIdx.x;
  int C_row_abs = threadIdx.y + BLOCK_SZ * blockIdx.y;
  FP Cval;
  ...
  if(C_row_abs < n && C_col_abs < m) {
    Cval = learning_rate * curr_derivative[C_row_abs];
    atomicAdd(&biasdeltas[C_row_abs], ::fdividef(Cval, batch_size));
  }
}

void update_biasdeltas(backprop_args_t* backprop_args_arr, int idx_in_batch, int batch_size, FP* d_biasdeltas, int n, int m, FP learning_rate, cudaStream_t* streams) {
  uint32_t curr_thread = idx_in_batch % NUM_STREAMS;
  int k;
  ...
  if(curr_thread == 0) {
    for(k=0; k<NUM_STREAMS; k++) {
      if(idx_in_batch + k >= batch_size) break;
      update_biasdeltas_gpu<<<Grid,Block,0,streams[k]>>>(backprop_args_arr[k].d_curr_derivative, batch_size, d_biasdeltas, n, m, learning_rate);
    }
  }
  #pragma omp barrier
}
...
void stochastic_backprop(void* backprop_params) {
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    if(curr_thread == 0) {
      for(k=0; k<NUM_STREAMS; k++) {
        if(idx_in_batch + k >= batch_size) break;
        blocked_gpu_matrixmult<<<Grid,Block,0,streams[k]>>>(args_arr[k].d_curr_derivative, layer_mtdt_arr[i].weights, args_arr[k].d_next_derivative, NULL, 1, next_dim, curr_dim, false, true);
      }
    }
    #pragma omp barrier
    
    update_deltas(args_arr, layer_mtdt_arr[i - 1].activations, idx_in_batch, batch_size, layer_mtdt_arr[i].deltas, curr_dim, next_dim, learning_rate, streams);
    update_biasdeltas(args_arr, idx_in_batch, batch_size, layer_mtdt_arr[i].biasdeltas, curr_dim, 1, learning_rate, streams);
  ...
  update_deltas(args_arr, layer_mtdt_arr[0].activations, idx_in_batch, batch_size, layer_mtdt_arr[1].deltas, layer_dims_arr[1], layer_dims_arr[0], learning_rate, streams);
  update_biasdeltas(args_arr, idx_in_batch, batch_size, layer_mtdt_arr[1].biasdeltas, layer_dims_arr[1], 1, learning_rate, streams);
}
...
__global__ void update_weights_biases(FP* weights, FP* deltas, FP* biasdeltas, FP batch_size, int n, int m) {
  // absolute position
  int C_col_abs = threadIdx.x + BLOCK_SZ * blockIdx.x;
  int C_row_abs = threadIdx.y + BLOCK_SZ * blockIdx.y;
  ...
  if(C_row_abs < n && C_col_abs < m) {
    if(C_col_abs == 0) {
      weights[C_row_abs * m] -= biasdeltas[C_row_abs];
      biasdeltas[C_row_abs] = 0;
    }
    else {
      weights[C_row_abs * m + C_col_abs] -= deltas[C_row_abs * (m - 1) + C_col_abs - 1];
      deltas[C_row_abs * (m - 1) + C_col_abs - 1] = 0;
    }
  }
}
  ...
  for(i=0; i<train_dataset->num_batches; i++) {
    ...
    if(curr_thread == 0) {
      ...
      for(j=1; j<num_layers; j++) {
        ...
        update_weights_biases<<<Grid,Block>>>(layer_mtdt_arr[j].weights, layer_mtdt_arr[j].deltas, layer_mtdt_arr[j].biasdeltas, (FP)actual_batch_size, layer_dims_arr[j], layer_dims_arr[j - 1] + 1);
      }
      // PRINT START
...
int main(int argc, char *argv[]) {
  ...
  uint8_t* train_images_raw = read_binary_file("../data/cxr/train_images");
  uint8_t* train_labels_raw = read_binary_file("../data/cxr/train_labels");
  ...
  uint8_t* test_images_raw = read_binary_file("../data/cxr/test_images");
  uint8_t* test_labels_raw = read_binary_file("../data/cxr/test_labels");
