...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  cudaMemcpyAsync(layer_outputs, d_layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP), cudaMemcpyDeviceToHost, *stream);
  exec_thread++;
  pthread_mutex_unlock(&lock);
  pthread_barrier_wait(&barrier);
  ...
  cudaMemcpyAsync(d_softmax_jacobian, softmax_jacobian, layer_dim * layer_dim * sizeof(FP), cudaMemcpyHostToDevice, *stream);
  exec_thread++;
  pthread_mutex_unlock(&lock);
  pthread_barrier_wait(&barrier);
  if(curr_thread == 0) exec_thread = 0;
  ...
  cudaMemcpyAsync(d_layer_output_single, layer_output_single, layer_dim * sizeof(FP), cudaMemcpyHostToDevice, *stream);
  exec_thread++;
  pthread_mutex_unlock(&lock);
  pthread_barrier_wait(&barrier);
  if(curr_thread == 0) exec_thread = 0;
...
FP* calc_relu_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  cudaMemcpyAsync(d_relu_derivative, relu_derivative, layer_dim * sizeof(FP), cudaMemcpyHostToDevice, *stream);
  exec_thread++;
  pthread_mutex_unlock(&lock);
  pthread_barrier_wait(&barrier);
  if(curr_thread == 0) exec_thread = 0;
