...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream) {
  int i, j;
  FP* layer_outputs;
  cudaMallocHost((void**)&layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP));
  cudaMemcpy(layer_outputs, d_layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP), cudaMemcpyDeviceToHost);
  FP* layer_output_single;
  cudaMallocHost((void**)&layer_output_single, layer_dim * sizeof(FP));
  ...
  FP* softmax_output;
  cudaMallocHost((void**)&softmax_output, layer_dim * sizeof(FP));
  for(i=0; i<layer_dim; i++) {
    softmax_output[i] = (FP)exp(layer_output_single[i] - max_val - (FP)log(sum));
  }

  FP* softmax_jacobian;
  cudaMallocHost((void**)&softmax_jacobian, layer_dim * layer_dim * sizeof(FP));
  ...
  cudaMallocHost((void**)&softmax_derivative, layer_dim * sizeof(FP));
  ...
  cudaFreeHost(layer_outputs);
  cudaFreeHost(layer_output_single);
  cudaFreeHost(softmax_output);
  cudaFreeHost(softmax_jacobian);
...
FP* calc_relu_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim) {
  int i, j;
  FP* layer_outputs;
  cudaMallocHost((void**)&layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP));
  cudaMemcpy(layer_outputs, d_layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP), cudaMemcpyDeviceToHost);
  ...
  FP* relu_derivative;
  cudaMallocHost((void**)&relu_derivative, layer_dim * sizeof(FP));
  for(i=0; i<layer_dim; i++) {
...
void* stochastic_backprop(void* arguments) {
  backprop_args_t* args = (backprop_args_t*)arguments;
  FP* xentropy_derivative = args->xentropy_derivative;
  int idx_in_batch = args->idx_in_batch;
  uint32_t batch_size = args->batch_size;
  layer_metadata_t* layer_mtdt_arr = args->layer_mtdt_arr;
  uint32_t* layer_dims_arr = args->layer_dims_arr;
  uint32_t num_layers = args->num_layers;
  FP learning_rate = args->learning_rate;
  ...
  FP* curr_derivative;
  cudaMallocHost((void**)&curr_derivative, MNIST_LABELS * sizeof(FP));
  int i, j;
  j = 0;
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    cudaMallocHost((void**)&next_derivative, next_dim * sizeof(FP));
    cudaMalloc((void**)&d_next_derivative, next_dim * sizeof(FP));  
    ...
    if(i == num_layers - 1)
      ...
      activation_derivative = calc_relu_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim);
    ...
    cudaFreeHost(curr_derivative);
    cudaFree(d_curr_derivative);

    cudaMallocHost((void**)&curr_derivative, (next_dim - 1) * sizeof(FP));
    for(j=1; j<next_dim; j++) {
      curr_derivative[j-1] = next_derivative[j];
    }
    cudaMalloc((void**)&d_curr_derivative, (next_dim - 1) * sizeof(FP));
    cudaMemcpy(d_curr_derivative, curr_derivative, (next_dim - 1) * sizeof(FP), cudaMemcpyHostToDevice);
    
    cudaFreeHost(next_derivative);
    cudaFree(d_next_derivative);
    cudaFreeHost(activation_derivative);
  }
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  backprop_args_t* backprop_args_arr = (backprop_args_t*)malloc(sizeof(backprop_args_t) * NUM_STREAMS);
  ...
  for(i=0; i<train_dataset->num_batches; i++) {
    ...
    FP* xentropy_derivative = (FP*)malloc(MNIST_LABELS * actual_batch_size * sizeof(FP));
    ...
    for(j=0; j<actual_batch_size; j+=NUM_STREAMS) {
      for(k=0; k<NUM_STREAMS; k++) {
        ...
        backprop_args_arr[k].xentropy_derivative = xentropy_derivative;
        backprop_args_arr[k].idx_in_batch = j + k;
        backprop_args_arr[k].batch_size = actual_batch_size;
        backprop_args_arr[k].layer_mtdt_arr = layer_mtdt_arr;
        backprop_args_arr[k].layer_dims_arr = layer_dims_arr;
        backprop_args_arr[k].num_layers = num_layers;
        backprop_args_arr[k].learning_rate = learning_rate;
        backprop_args_arr[k].stream = &streams[k];
        if(pthread_create(&threads[k], NULL, stochastic_backprop, (void*)&backprop_args_arr[k])) {
