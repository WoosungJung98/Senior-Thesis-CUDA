...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim) {
  int i, j;
  FP* layer_outputs = (FP*)malloc((layer_dim - 1) * batch_size * sizeof(FP));
...
__global__ void update_deltas_gpu(FP* curr_derivative, FP* activations, int idx_in_batch, int batch_size, FP* deltas, int n, int m, FP learning_rate) {
  ...
  if(C_row_abs < n && C_col_abs < m)
    deltas[C_row_abs * m + C_col_abs] += learning_rate * curr_derivative[C_row_abs] * actv_value;
}

void update_deltas(FP* d_curr_derivative, FP* d_activations, int idx_in_batch, int batch_size, FP* d_deltas, int n, int m, FP learning_rate) {
  dim3 Block(BLOCK_SZ, BLOCK_SZ);
  dim3 Grid;
...
void stochastic_backprop(FP* xentropy_derivative, int idx_in_batch, uint32_t batch_size, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    next_derivative = (FP*)malloc(next_dim * sizeof(FP));
    ...
    if(i == num_layers - 1)
      activation_derivative = calc_softmax_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim);
    else
      activation_derivative = calc_relu_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim);
    
    // COMPUTATION DONE ON GPU
    blocked_gpu_matrixmult<<<Grid,Block>>>(d_curr_derivative, layer_mtdt_arr[i].weights, d_next_derivative, NULL, 1, next_dim, curr_dim, false);
    ...
    update_deltas(d_curr_derivative, layer_mtdt_arr[i - 1].activations, idx_in_batch, batch_size, layer_mtdt_arr[i].deltas, curr_dim, next_dim, learning_rate);
   
    free(curr_derivative);
    cudaFree(d_curr_derivative);

    curr_derivative = (FP*)malloc((next_dim - 1) * sizeof(FP));
  ...
  update_deltas(d_curr_derivative, layer_mtdt_arr[0].activations, idx_in_batch, batch_size, layer_mtdt_arr[1].deltas, layer_dims_arr[1], layer_dims_arr[0] + 1, learning_rate);
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  for(i=0; i<train_dataset->num_batches; i++) {
    ...
    for(j=0; j<actual_batch_size; j++) {
      stochastic_backprop(xentropy_derivative, j, actual_batch_size, layer_mtdt_arr, layer_dims_arr, num_layers, learning_rate);
    }
