...
FP* calc_categorical_xentropy(FP* output_layer_outputs, FP* output_layer_activations, uint32_t batch_size, FP* d_labels, cat_xentropy_args_t* args) {
  ...
  FP* max_arr = args->max_arr;
  FP* sum_arr = args->sum_arr;
...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  FP* layer_outputs = args->layer_outputs;
  FP* layer_output_single = args->layer_output_single;
  FP* softmax_output = args->softmax_output;
  FP* softmax_jacobian = args->softmax_jacobian;
  ...
  int i, j;
  cudaMemcpy(layer_outputs, d_layer_outputs, (layer_dim - 1) * batch_size * sizeof(FP), cudaMemcpyDeviceToHost);

  layer_output_single[0] = 1;
  j = 1;
  for(i=idx_in_batch; i<(layer_dim - 1) * batch_size; i+=batch_size, j++) {
...
FP* calc_relu_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, activation_deriv_args_t* args) {
  FP* layer_outputs = args->layer_outputs;
  FP* layer_output_single = args->layer_output_single;
  FP* relu_derivative = args->relu_derivative;
  ...
  cudaMemcpy(d_relu_derivative, relu_derivative, layer_dim * sizeof(FP), cudaMemcpyHostToDevice);
...
void stochastic_backprop(FP* xentropy_derivative, int idx_in_batch, uint32_t batch_size, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr,
                         uint32_t num_layers, FP learning_rate, cudaStream_t* stream, backprop_args_t* args, activation_deriv_args_t* actv_deriv_args) {
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    if(i == num_layers - 1)
      ...
      d_activation_derivative = calc_relu_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim, actv_deriv_args);
    ...
    cudaMemcpy(activation_derivative, d_activation_derivative, next_dim * sizeof(FP), cudaMemcpyDeviceToHost);
    cudaMemcpy(next_derivative, d_next_derivative, next_dim * sizeof(FP), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(d_curr_derivative, curr_derivative, (next_dim - 1) * sizeof(FP), cudaMemcpyHostToDevice);
  }
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  backprop_args_t backprop_args;
  cudaMallocHost((void**)&backprop_args.curr_derivative, (max_dim - 1) * sizeof(FP));
  cudaMalloc((void**)&backprop_args.d_curr_derivative, (max_dim - 1) * sizeof(FP));
  cudaMallocHost((void**)&backprop_args.next_derivative, max_dim * sizeof(FP));
  cudaMalloc((void**)&backprop_args.d_next_derivative, max_dim * sizeof(FP));
  cudaMallocHost((void**)&backprop_args.activation_derivative, max_dim * sizeof(FP));

  activation_deriv_args_t actv_deriv_args;
  cudaMallocHost((void**)&actv_deriv_args.layer_outputs, (max_dim - 1) * train_dataset->batches[0].size * sizeof(FP));
  cudaMallocHost((void**)&actv_deriv_args.layer_output_single, max_dim * sizeof(FP));
  cudaMallocHost((void**)&actv_deriv_args.softmax_output, max_dim * sizeof(FP));
  cudaMallocHost((void**)&actv_deriv_args.softmax_jacobian, max_dim * max_dim * sizeof(FP));
  cudaMalloc((void**)&actv_deriv_args.d_softmax_jacobian, max_dim * max_dim * sizeof(FP));
  cudaMalloc((void**)&actv_deriv_args.d_layer_output_single, max_dim * sizeof(FP));
  cudaMalloc((void**)&actv_deriv_args.d_softmax_derivative, max_dim * sizeof(FP));
  cudaMallocHost((void**)&actv_deriv_args.relu_derivative, max_dim * sizeof(FP));
  cudaMalloc((void**)&actv_deriv_args.d_relu_derivative, max_dim * sizeof(FP));
  ...
  cudaMallocHost((void**)&xentropy_args.max_arr, train_dataset->batches[0].size * sizeof(FP));
  cudaMallocHost((void**)&xentropy_args.sum_arr, train_dataset->batches[0].size * sizeof(FP));
  ...
  for(i=0; i<train_dataset->num_batches; i++) {
    ...
    for(j=0; j<actual_batch_size; j+=NUM_STREAMS) {
      for(k=0; k<NUM_STREAMS; k++) {
        ...
        stochastic_backprop(xentropy_derivative, j + k, actual_batch_size, layer_mtdt_arr, layer_dims_arr, num_layers, learning_rate, &streams[k], &backprop_args, &actv_deriv_args);
      }
    }
  ...
  cudaFreeHost(backprop_args.curr_derivative);
  cudaFree(backprop_args.d_curr_derivative);
  cudaFreeHost(backprop_args.next_derivative);
  cudaFree(backprop_args.d_next_derivative);
  cudaFreeHost(backprop_args.activation_derivative);

  cudaFreeHost(actv_deriv_args.layer_outputs);
  cudaFreeHost(actv_deriv_args.layer_output_single);
  cudaFreeHost(actv_deriv_args.softmax_output);
  cudaFreeHost(actv_deriv_args.softmax_jacobian);
  cudaFree(actv_deriv_args.d_softmax_jacobian);
  cudaFree(actv_deriv_args.d_layer_output_single);
  cudaFree(actv_deriv_args.d_softmax_derivative);
  cudaFreeHost(actv_deriv_args.relu_derivative);
  cudaFree(actv_deriv_args.d_relu_derivative);

  cudaFree(xentropy_args.d_max_arr);
  cudaFree(xentropy_args.d_sum_arr);
  cudaFree(xentropy_args.d_xentropy_arr);
  cudaFreeHost(xentropy_args.max_arr);
  cudaFreeHost(xentropy_args.sum_arr);
...
void test_mnist(mnist_dataset_t* test_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers) {
  ...
  cudaMallocHost((void**)&xentropy_args.max_arr, test_dataset->batches[0].size * sizeof(FP));
  cudaMallocHost((void**)&xentropy_args.sum_arr, test_dataset->batches[0].size * sizeof(FP));
