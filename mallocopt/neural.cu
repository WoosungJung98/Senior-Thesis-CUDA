...
FP* calc_softmax_derivative(FP* d_layer_outputs, int idx_in_batch, uint32_t batch_size, uint32_t layer_dim, cudaStream_t* stream, activation_deriv_args_t* args) {
  ...
  FP* softmax_derivative = args->softmax_derivative;
...
void stochastic_backprop(void* backprop_params) {
  ...
  for(i=num_layers - 1; i>=2; i--) {
    ...
    if(i == num_layers - 1)
      activation_derivative = calc_softmax_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim, stream, actv_deriv_args);
    else
      activation_derivative = calc_relu_derivative(layer_mtdt_arr[i - 1].outputs, idx_in_batch, batch_size, next_dim, stream, actv_deriv_args);
...
void mini_batch_gradient_descent(void* grad_descent_params) {
  ...
  uint32_t num_layers = params->num_layers;
  FP learning_rate = params->learning_rate;
  FP* d_labels = params->d_labels;
  cat_xentropy_args_t* xentropy_args = params->xentropy_args;
  ...
  backprop_args_t* backprop_args_arr = params->backprop_args_arr;
  ...
  backprop_params_arr[curr_thread].args = &backprop_args_arr[curr_thread];
  backprop_params_arr[curr_thread].actv_deriv_args = &actv_deriv_args_arr[curr_thread];
...
void train_mnist(mnist_dataset_t* train_dataset, layer_metadata_t* layer_mtdt_arr, uint32_t* layer_dims_arr, uint32_t num_layers, FP learning_rate) {
  ...
  for(i=0; i<NUM_STREAMS; i++) {
    cudaMallocHost((void**)&backprop_args_arr[i].curr_derivative, max_dim * sizeof(FP));
    cudaMalloc((void**)&backprop_args_arr[i].d_curr_derivative, max_dim * sizeof(FP));
    cudaMallocHost((void**)&backprop_args_arr[i].next_derivative, (max_dim + 1) * sizeof(FP));
  ...
  for(i=0; i<NUM_STREAMS; i++) {
    cudaMallocHost((void**)&actv_deriv_args_arr[i].layer_outputs, max_dim * train_dataset->batches[0].size * sizeof(FP));
    cudaMallocHost((void**)&actv_deriv_args_arr[i].layer_output_single, max_dim * sizeof(FP));
    cudaMallocHost((void**)&actv_deriv_args_arr[i].softmax_output, max_dim * sizeof(FP));
    cudaMallocHost((void**)&actv_deriv_args_arr[i].softmax_jacobian, max_dim * max_dim * sizeof(FP));
    ...
    cudaMallocHost((void**)&actv_deriv_args_arr[i].softmax_derivative, max_dim * sizeof(FP));
    cudaMalloc((void**)&actv_deriv_args_arr[i].d_softmax_derivative, max_dim * sizeof(FP));
    cudaMallocHost((void**)&actv_deriv_args_arr[i].relu_derivative, max_dim * sizeof(FP));
  }
  ...
  #pragma omp parallel shared(train_dataset, layer_mtdt_arr, layer_dims_arr, num_layers, learning_rate, d_labels, xentropy_args, output_layer_activations, d_xentropy_derivative, xentropy_derivative, backprop_params_arr, streams, backprop_args_arr, actv_deriv_args_arr, grad_descent_params_arr) default(none)
  {
    int tid = omp_get_thread_num();
    grad_descent_params_arr[tid].train_dataset = train_dataset;
    grad_descent_params_arr[tid].layer_mtdt_arr = layer_mtdt_arr;
    grad_descent_params_arr[tid].layer_dims_arr = layer_dims_arr;
    grad_descent_params_arr[tid].num_layers = num_layers;
    grad_descent_params_arr[tid].learning_rate = learning_rate;
    grad_descent_params_arr[tid].d_labels = d_labels;
    grad_descent_params_arr[tid].xentropy_args = &xentropy_args;
    grad_descent_params_arr[tid].output_layer_activations = output_layer_activations;
    grad_descent_params_arr[tid].d_xentropy_derivative = d_xentropy_derivative;
    grad_descent_params_arr[tid].xentropy_derivative = xentropy_derivative;
    grad_descent_params_arr[tid].backprop_params_arr = backprop_params_arr;
    grad_descent_params_arr[tid].streams = streams;
    grad_descent_params_arr[tid].backprop_args_arr = backprop_args_arr;
    grad_descent_params_arr[tid].actv_deriv_args_arr = actv_deriv_args_arr;
    grad_descent_params_arr[tid].curr_thread = tid;
    mini_batch_gradient_descent((void*)&grad_descent_params_arr[tid]);
  }
  ...
  for(i=0; i<NUM_STREAMS; i++) {
    cudaFreeHost(actv_deriv_args_arr[i].layer_outputs);
    cudaFreeHost(actv_deriv_args_arr[i].layer_output_single);
    cudaFreeHost(actv_deriv_args_arr[i].softmax_output);
    cudaFreeHost(actv_deriv_args_arr[i].softmax_jacobian);
    cudaFree(actv_deriv_args_arr[i].d_softmax_jacobian);
    cudaFree(actv_deriv_args_arr[i].d_layer_output_single);
    cudaFreeHost(actv_deriv_args_arr[i].softmax_derivative);
    cudaFree(actv_deriv_args_arr[i].d_softmax_derivative);
    cudaFreeHost(actv_deriv_args_arr[i].relu_derivative);
  }
